...
def prior_N(n_min=50, n_max=400):
...
generative_model = bf.simulation.GenerativeModel(prior,simulator,name='schubert')
...
amortizer = bf.amortizers.AmortizedPosterior(inference_net, summary_net, name='schubert')

trainer = bf.trainers.Trainer(
    amortizer=amortizer, 
    generative_model=generative_model, 
    configurator=configurator,
    checkpoint_path='./checkpoints/schubert_50_400_py_go')

num_val = 300
val_sims = generative_model(num_val)

h = trainer.train_online(epochs=98, iterations_per_epoch=1000, batch_size=128, validation_sims=val_sims)

f = bf.diagnostics.plot_losses(h['train_losses'], h['val_losses'])
plt.savefig('./schubert/loss.png')



def prior_N(n_min=400, n_max=400):
    """A prior fo]r the number of observation (will be called internally at each backprop step)."""
    return  np.random.randint(n_min, n_max+1)
...
plt.savefig('./schubert/schubert_recovery.png')
...
plt.savefig('./schubert/schubert_ecdf.png')
...
plt.savefig('./schubert/schubert_ecdf_all.png')
...
plt.savefig('./schubert/schubert_SBC.png')
...
plt.savefig('./schubert/schubert_recovery_blue.png')
