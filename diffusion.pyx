...
cdef api double diffusion_trial(double v, double sv, double zr, double szr, double a, 
    ...
    #zr = zr - 0.5*szr + szr * random_uniform()
    #zr =  zr -.5*np.sqrt(12)*szr + np.sqrt(12)*szr * np.random.uniform()
    #zr = zr - 0.5 * sqrt(12) * szr + sqrt(12) * szr * rand_uniform()
    zr = zr - 0.5* sqrt(12)*szr + sqrt(12) * szr * random_uniform()

    # draw zr from block
    ndt = ndt - 0.5*sqrt(12) * sndt + sqrt(12) * sndt * random_uniform() # draw ndt from block
