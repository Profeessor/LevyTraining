...
def simulate_diffusion_2_conds(theta, n_trials, dt=0.001, max_steps=15000):
    ...
    n_trials_c1_ass     = n_trials[0]
    n_trials_c1_non_ass = n_trials[1]

    n_trials_c2_ass     = n_trials[2]
    n_trials_c2_non_ass = n_trials[3]

    
    
    v1,v2,v3,v4,zr, a, ndt1, ndt2, ndt3, ndt4,sndt,alpha = theta
    rt_c1_ass     = simulate_diffusion_condition(n_trials_c1_ass     ,v1,0.0,zr,0.0, a, ndt1,sndt,alpha, dt, max_steps)
    rt_c1_non_ass = simulate_diffusion_condition(n_trials_c1_non_ass, v2,0.0,zr,0.0, a, ndt2,sndt,alpha, dt, max_steps)
    rt_c2_ass     = simulate_diffusion_condition(n_trials_c2_ass,     v3,0.0,zr,0.0, a, ndt3,sndt,alpha, dt, max_steps)
    rt_c2_non_ass = simulate_diffusion_condition(n_trials_c2_non_ass, v4,0.0,zr,0.0, a, ndt4,sndt,alpha, dt, max_steps)

    rts = np.concatenate((rt_c1_ass, rt_c1_non_ass,rt_c2_ass,rt_c2_non_ass))
    
    # Assign conditions
    cond_arr = np.concatenate((np.zeros(n_trials_c1_ass), np.ones(n_trials_c1_non_ass),np.ones(n_trials_c2_ass)*2,np.ones(n_trials_c2_non_ass)*3))
